...
var containerHeightLimit = [1000, 1200];     // defines after which height different config parameters are used
...
function exportDataToCsv() {
    ...
    console.log('[Chart Widget] Exporting data to CSV with custom filename');
    ...
    sortedTimestamps.forEach((timestamp) => {
        ...
        self.ctx.data.forEach((series) => {
            ...
            if (value !== undefined) {
                const decimals = series.dataKey.decimals || self.ctx.decimals || 2;
    ...
    console.log('[Chart Widget] CSV export completed with filename:', filename);
}

// Override CSV export methods
function overrideCsvExport() {
    ...
    if (self.exportCsv || self.exportCSV) {
        ...
        self[exportMethod] = function() {
            console.log('[Chart Widget] CSV export intercepted!');
            ...
            window.saveAs = function(blob, filename) {
                console.log('[Chart Widget] saveAs intercepted, original filename:', filename);
    ...
    const checkInterval = setInterval(() => {
        if (window.saveAs && !window.saveAs._customOverride) {
            const originalSaveAs = window.saveAs;
            ...
            // Mark our override
            window.saveAs._customOverride = true;
            
            // Keep reference to original
            window.saveAs._original = originalSaveAs;
        }
    }, 500);
    ...
    if (self.onDestroy) {
        ...
        self.onDestroy = function() {
            ...
            if (window.saveAs && window.saveAs._original) {
                window.saveAs = window.saveAs._original;
            }
...
self.onInit = function() {
    ...
    // Setup menu buttons including CSV export
